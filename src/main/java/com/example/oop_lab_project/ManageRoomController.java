...
public class ManageRoomController {
        ...
        try (Connection connection = connectDB(); PreparedStatement preparedStatement = connection.prepareStatement(query)) {
            ...
            while (resultSet.next()) {
                Appointment appointment = new Appointment(
                        ...
                        resultSet.getInt("bed_number") // Ensure the column name matches exactly
                );
    ...
    private void handleUpdate(ActionEvent event) {
        ...
        if (room.isEmpty()) {
            showAlert(Alert.AlertType.ERROR, "Form Error", "Please enter a room number.");
            return;
        }

        String checkRoomQuery = "SELECT COUNT(*) FROM appointment WHERE room = ?";
        String updateQuery = "UPDATE appointment SET room = ? WHERE id = ?";

        try (Connection connection = connectDB();
             PreparedStatement checkRoomStmt = connection.prepareStatement(checkRoomQuery);
             PreparedStatement updateStmt = connection.prepareStatement(updateQuery)) {

            checkRoomStmt.setString(1, room);
            ResultSet resultSet = checkRoomStmt.executeQuery();
            if (resultSet.next() && resultSet.getInt(1) > 0) {
                showAlert(Alert.AlertType.ERROR, "Update Error", "This room is already assigned to another patient.");
                return;
            }
            ...
            if (result > 0) {
                showAlert(Alert.AlertType.INFORMATION, "Success", "Room number assigned successfully.");
                loadAppointments();
            } else {
                showAlert(Alert.AlertType.ERROR, "Update Error", "Failed to assign room number.");
            }

        } catch (SQLException e) {
            e.printStackTrace();
            showAlert(Alert.AlertType.ERROR, "Database Error", "An error occurred while updating the room number.");
        }
    }
